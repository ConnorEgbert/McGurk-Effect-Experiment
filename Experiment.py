...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/x/Documents/McGurk-Effect-Experiment/Experiment.py',
...
video_noise = visual.NoiseStim(
    ...
    ori=0, pos=(0,0), size=None, sf=None, phase=0.0,
    color=[1,1,1], colorSpace='rgb', opacity=.5, blendmode='avg', contrast=1.0,
    texRes=128,
    noiseType='Isotropic', noiseElementSize=0.015, noiseBaseSf=.01,
...
test_video = visual.MovieStim3(
    win=win, name='test_video',
    noAudio = True,
    filename='BA.mov',
    ori=0, pos=(0, 0), opacity=1,
    depth=-1.0,
    )
test_audio = visual.MovieStim3(
    win=win, name='test_audio',
    noAudio = False,
    filename='BA.mov',
    ori=0, pos=(0, 0), opacity=1,
    size=(0, 0),
...
test_noise = visual.NoiseStim(
    win=win, name='test_noise',
    noiseImage=None, mask=None,
...
audio_stim = test_audio


def show_test(video_file, audio_file, video_noise_level, audio_noise_level):
    video_stim.setMovie(video_file)
    audio_stim.setMovie(audio_file)
    video_stim.seek(0)
    audio_stim.seek(0)
    video_stim.play()
    audio_stim.play()
    while video_stim.status != visual.FINISHED and audio_stim.status != visual.FINISHED:
        video_stim.draw()
        #audio_stim.draw()
        win.flip()
...
# keep track of which components have finished
TestingComponents = [test_video, test_audio, test_noise]
...
while continueRoutine:
    ...
    # *test_audio* updates
    if t >= 0.0 and test_audio.status == NOT_STARTED:
        # keep track of start time/frame for later
        test_audio.tStart = t
        test_audio.frameNStart = frameN  # exact frame index
        test_audio.setAutoDraw(True)
    frameRemains = 0.0 + 0- win.monitorFramePeriod * 0.75  # most of one frame period left
    if test_audio.status == STARTED and t >= frameRemains:
        test_audio.setAutoDraw(False)
    
    # *test_noise* updates
    if t >= 0.0 and test_noise.status == NOT_STARTED:
        # keep track of start time/frame for later
        test_noise.tStart = t
        test_noise.frameNStart = frameN  # exact frame index
        test_noise.setAutoDraw(True)
    frameRemains = 0.0 + 0- win.monitorFramePeriod * 0.75  # most of one frame period left
    if test_noise.status == STARTED and t >= frameRemains:
        test_noise.setAutoDraw(False)
    if test_noise.status == STARTED:
        if test_noise._needBuild:
            test_noise.buildNoise()
